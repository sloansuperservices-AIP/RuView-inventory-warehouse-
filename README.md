...
| [**Package-Flow — warehouse package tracker**](v2/crates/cog-package-flow/) | Turns an **existing shop-floor AP grid** (~50 ft spacing) into a mesh of **directional RF tripwires** — no tags, cameras, or barcodes. Adjacent bistatic-link "sensing lines" form gates; firing order gives crossing direction; zones do package **flow accounting** (per-gate forward/reverse counts, per-zone net flow, coarse tote/carton/pallet/forklift size class). Counts **flow events, not static inventory** (a still box is invisible to WiFi CSI — honest by design). Ships a deterministic synthetic warehouse (`simulate`/`serve`, `synthetic=true`) + a self-contained Axum **live dashboard**; real `run` path polls `wifi-densepose-sensing-server` (`data_gated` until per-site calibrated). Pure Rust, mirrors `cog-person-count`. 17 tests. [ADR-266](docs/adr/ADR-266-wifi-csi-package-flow-tracker.md). Try: `cargo run -p cog-package-flow -- serve` → http://127.0.0.1:8787/ |
